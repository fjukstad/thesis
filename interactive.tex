...
The \gls{wgcna} Shiny app\footnote{Online a shiny.etriks.org/wgcna} is an
interactive application for performing, and exploring results from, \gls{wgcna}.
The online version allows users to explore two demo datasets, and it is possible
to download the application and change out the datasets locally. In short it is
a web implementation of the \gls{wgcna} R package that allows users without any
R experience perform \gls{wgcna}. It is developed and maintained by the eTRIKS
platform.\cite{bussery2018etriks}
...
can navigate through and download. They provide a \gls{rest} \gls{api} that
...
Reactome is an open-source peer-reviewed online knowledgeabse of biomolecular
...
made an API open for developers, but are planning to do so. Libraries such as
KEGGViewer\cite{villaveces2014keggviewer} allow developers to integrate pathway
visualization maps in web appplications, but while it generates pathway maps
from the \gls{kegg} it does not provide the same visual cues as the static
\gls{kegg} pathway maps.  enRoute\cite{partl2012enroute} is a desktop
