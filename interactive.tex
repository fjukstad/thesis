Explorative analyses are essential for understanding high-throughput datasets.
Through the development of several data exploration applications, we have
iteratively developed an approach to build such applications. 
We demonstrate the usefulness of the approach through a set of different
applications for exploring transcriptional profiles from the \gls{nowac} cohort.
While these are specialized applications, we show how the design patterns and
ideas can be used in a wide range of disciplines. 
...
through integration with online databases and interactive visualizations. 
...
From collaboration with researchers in systems epidemiology and biology we
identified a set of requirements that the applications should satisfy: 
...
\section{Design}\label{challengeref} 
...
find these patterns. Such software is usually written statistical programming
...
the last years. Kvik provides contains 
\begin{itemize}
    \item an interface to the R statistical programming language, both as a
        stand-alone service, a client library, and through an
        OpenCPU\cite{opencpu} server  ; 
    \item an R-based pipelining tool that allows useres to specify and run
        statistical analysis pipelines in R;  
    \item an interface with online databases such as MsigDB and KEGG; and
\end{itemize} 

In addition to these packages, Kvik also contains stand-alone services that
provide interfaces to the R programming language and the online databases. We
outline these services in \ref{micrservices}.  
In addition to these packages it In addition it also contains a Javascript
package for visualizing KEGG pathways using d3.
