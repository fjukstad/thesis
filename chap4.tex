In this chapter we outline our approach for building interactive data
...
%These
%functions are: $get(genes)$, $genes()$, $fc(genes)$ and $pvalues(genes)$.
%$get(genes)$ retrieves all information available for the given genes. $genes()$
%returns a list of all of the genes in the dataset. $fc(genes)$ returns the
%fold change for the selected genes. $pvalues(genes)$ returns the $p$-values
%for the given genes. 
Researchers only
