...
% bioinformatics/cs
% - r-package, approach for others in same situation? 
% - nothing new, just application o
% 
% significance in clinical
% - easier to get started for users
% - reproducible science,
% - no more "where did my script go?" bs 
% - 
% 
% limitations to related
% - related? Scripts that do similar things, no system that I know of that
%   provide an R package? Maybe check paper. 
% - git setup not that good, but hey we're at least versioning stuff
% - R is probably a limitation? Or a 
% - researchers need to learn git & R, but fucking hell it's 2018 everyone
%   should know how to code. 

% Pippeline (preprocessing etc.) now works for RNA-seq, Methylation, microRNA
